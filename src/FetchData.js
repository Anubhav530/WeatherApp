...
const FetchData = () => {
    ...
    useEffect( () => {
        const fetchApi = async () => {
            ...
            { resJson.cod == 200 ? 
                (
                    <>
                        ...
                        { setIcon('http://openweathermap.org/img/wn/'+ resJson.weather[0].icon +'.png') }
                        ...
                        { console.log(new Date(new Date().getTime() + (new Date().getTimezoneOffset() * 60000) +  (3600000*(dateTime/3600))).toLocaleString()) }
            ...
            else if(weather.main=="Dust" || "Mist" || "Smoke" || "Haze" || "Fog" || "Sand" || "Ash" || "Squall" || "Tornado"){
                weatherbg=weather.main;
                { setBg(weather.main) }
            }
        ...
        const datetime = (new Date(new Date().getTime() + (new Date().getTimezoneOffset() * 60000) +  (3600000*(dateTime/3600))).toLocaleString())
        return(
        <>
        <div className= { "setBgImage "+bg }>
        <div className="box mt-4 ">
                <div className="inputData mt-3">
                    <input type="search" placeholder="Search" className="inputField" onChange={(event) => { setSearch(event.target.value) }} value={search} />
                </div>
        ...
        {!city ? (
            ...
            <div className="info">
                ...
                <span className="capitalize"> <img src = { icon } /> { weather.description } </span>
                
                <h1 className="temp">{city.temp} &deg;C</h1>
                
                <h3 className="tempmin_max">Feels like: {city.feels_like} &deg;C | Min: {city.temp_min} &deg;C | Max: {city.temp_max} &deg;C</h3>
            </div>
            </>
        )
